...
class Vis_Circle2(Plugin):
    uniqueness = "not_unique"

    def __init__(self, g_pool,radius=5,color=(1.0,1.0,0.0,1.0),thickness=2,fill=True):
    ...
    def update(self,frame,events):
        ...
        #        print(pt['timestamp'], pt['index'], pt['norm_pos']) #(A)
        for pt in pts:
            transparent_circle(frame.img, pt, radius=self.radius, color=(self.b, self.g, self.r, self.a), thickness=thickness)
        #print ('frame', frame.timestamp, frame.index)    

    def init_gui(self):
        # initialize the menu
        self.menu = ui.Scrolling_Menu('Gaze Circle2')
