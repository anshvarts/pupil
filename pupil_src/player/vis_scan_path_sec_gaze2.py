...
class Vis_Scan_Path2(Plugin):
    """docstring
    using this plugin will extend the recent_gaze_positions by x extra dots from previous frames.
    This vertion of plugin does not lock recent gaze points onto pixels. #(A)
    """

    def __init__(self, g_pool,timeframe=2.0):
    ...
    def update(self,frame,events):
        ...
        succeeding_frame = frame.index-(frame.index-1) == 1 #(A) here is cheeting since my video does not work well with previous/next frame
        ...
        if same_frame:
            # paused
            # re-use last result
            events['gaze_positions2'][:] = self.past_gaze_positions[:]
        else:
            # trim gaze that is too old
            if events['gaze_positions2']:
                now = events['gaze_positions2'][0]['timestamp']
                cutoff = now-self.timeframe
                #updated_past_gaze = [g for g in updated_past_gaze if g['timestamp']>cutoff]
                self.past_gaze_positions[:] = [g for g in self.past_gaze_positions[:] if g['timestamp']>cutoff]
            #inject the scan path gaze points into recent_gaze_positions
            #events['gaze_positions'][:] = updated_past_gaze + events['gaze_positions'] (I will not use Update since I don't need to use optical flow)

            #draw a circle abount first point 
            pts = [denormalize(pt['norm_pos'],frame.img.shape[:-1][::-1],flip_y=True) for pt in events.get('gaze_positions2',[]) if pt['confidence']>=self.g_pool.min_data_confidence]
            for pt in pts:
                transparent_circle(frame.img, pt, radius=15, color=(0.0,1.0,1.0,1.0), thickness=2)


            events['gaze_positions2'][:] = self.past_gaze_positions[:] + events['gaze_positions2']
            events['gaze_positions2'].sort(key=lambda x: x['timestamp']) #this may be redundant...

        #update info for next frame.
        self.prev_gray = gray_img
        self.prev_frame_idx = frame.index
        self.past_gaze_positions = events['gaze_positions2']

    def init_gui(self):
        # initialize the menu
        self.menu = ui.Scrolling_Menu('Scan Path2')
