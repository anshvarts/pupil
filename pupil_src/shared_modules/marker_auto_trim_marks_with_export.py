...
class Marker_Auto_Trim_Marks(Plugin):
    ...
    def surface_video_export(self,section): #(A)
        plugins1 = [p for p in self.g_pool.plugins if isinstance(p,Offline_Surface_Tracker)]
        plugins2 = [p for p in self.g_pool.plugins if isinstance(p,Video_Export_Launcher)]
        if plugins1:
            print("hello1")
            tracker = plugins1[0]
            my_surfaces = tracker.load_surface_definitions_from_file
            ...
            if plugins2:
                ...
                logger.info("exporting {!s}".format(section))
                self.g_pool.trim_marks.set(section)
                launcher.rec_name.value = "world_viz_surface3_section_{}-{}".format(*section)
                launcher.add_export()
        else:
            logger.warning("Please start Offline_Surface_Tracker Plugin for surface export.")
