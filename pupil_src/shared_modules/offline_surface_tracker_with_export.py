...
from offline_reference_surface_with_export import Offline_Reference_Surface #(A)
from exporter_surface import Global_Container #(A)
from math import sqrt

#(A) other plugins - needs to be checked what is needed
#added from exporter:
...
from file_methods import load_object #(A)
...
class Offline_Surface_Tracker_With_Export(Surface_Tracker): #(A)
    """
    ...
    def on_notify(self,notification):
        ...
        if notification['subject'] == 'min_data_confidence_changed':
            ...
            self.add_export(notification['range'],notification['export_dir'])#(A)
            print('new export will be added')
    ...
    def add_export(self,export_range,export_dir): #(A)
        ...
        plugins = [] #self.g_pool.plugins.get_initializers() (TEMP)
        #out_file_path=verify_out_file_path(self.rec_name,export_dir) (TEMP)
        out_file_path = os.path.join(export_dir, "world_surface3_viz.mp4")
        #process = Export_Process(target=export_surface, args=(should_terminate,frames_to_export,current_frame, rec_dir,user_dir,self.g_pool.min_data_confidence,start_frame,end_frame,plugins,out_file_path))
        self.export_surface(should_terminate,frames_to_export,current_frame, rec_dir,user_dir,self.g_pool.min_data_confidence,start_frame,end_frame,plugins,out_file_path)
        #self.new_export = process
        #self.launch_export(self.new_export)
        #self.new_export = None

    def launch_export(self, new_export): #(A)
    ...
    def export_surface(self,should_terminate, frames_to_export, current_frame, rec_dir, user_dir, min_data_confidence,
        ...
        #plugin_tracker = [p for p in g_pool.plugins if isinstance(p,Offline_Surface_Tracker)]
        
        #if plugin_tracker:
        print("hello hello")
        #tracker = plugin_tracker[0]
        #my_surfaces = tracker.load_surface_definitions_from_file #Offline_Surface_Tracker
        ...
        while frames_to_export.value > current_frame.value:
            ...
            try:
                new_frame = cap.get_frame()  #(A)
                cap.wait(new_frame)

                print(new_frame.timestamp, new_frame.index)

                surface.locate_from_cache(new_frame.index)

                g_pool.frame_texture = Named_Texture()
                g_pool.frame_texture.update_from_frame(new_frame)            

                new_frame = surface.homography(g_pool.frame_texture,new_frame,rec_dir) #(A) s.gl_display_in_window(self.g_pool.image_tex)
                
                print('after homography', new_frame.timestamp, new_frame.index)
                #print(new_frame.height)
                #print(new_frame.width)
                #print(new_frame.img)# = new_frame.height + 200
            ...
            writer.write_video_frame(new_frame)
