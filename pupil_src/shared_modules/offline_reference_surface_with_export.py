...
from pyglui.cygl.utils import Named_Texture #(A)

import logging
logger = logging.getLogger(__name__)

from reference_surface import Reference_Surface
#from reference_surface_with_export import Reference_Surface #(A) - needed?
...
class Offline_Reference_Surface(Reference_Surface):
    ...
    def homography(self,frame_texture,frame,rec_dir): #gl_display_in_window
        """
        here we map a selected surface onto a seperate window and buil it in a new frame for the AV_writer
        """
        if self.detected:
            self.close_window()
            self.open_window()#(640,640)
            #print(self._window)
            print(self.detected)
            ...
            for gp in self.gaze_on_srf:
                x=gp['norm_pos'][0]
                y=1-gp['norm_pos'][1]#*height/width
                scaled_gp=(x,y)
                draw_points_norm([scaled_gp],color=RGBA(0.0,0.0,1.0,1.0), size=40)
            

            #adjust_gl_view(width,height)

            image = glReadPixels(0, 0, 640, 640, GL_RGB, GL_BYTE)
            #print(image)
            
            glfwSwapBuffers(self._window)
            glfwMakeContextCurrent(active_window)
            
            new_frame=Frame(frame.timestamp,image,frame.index)
            self.close_window()
